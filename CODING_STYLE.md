...
**Действующая политика — раздел «Явная null-политика» ниже (решение
пользователя 2026-09-02).** Разделы после неё — классификация легаси и техника
миграции.

---

## Явная null-политика (#NULL-EXPLICIT, решение пользователя 2026-09-02)

В коде запрещены неявные null-операторы: всё явное и прозрачное.
Исключения — обоснованны (inline-комментарий).

### 1. `!!` — ЗАПРЕЩЁН везде

`x!!` — это неявный NPE: падение без контекста и сообщения. Замены:

- **Явная проверка + ранний возврат / фолбэк с логом** (если null возможен —
  асинхронность, жизненный цикл):

```kotlin
val p = prefs
if (p == null) {
    AppLog.w(TAG, "prefs не готовы — пропускаем")
    return
}
```

- **`requireNotNull(x) { "обоснование" }` / `checkNotNull(x) { "обоснование" }`** —
  когда null невозможен по контракту (инвариант проверен выше, состояние
  инициализировано в этой же функции, ключ взят из `map.keys`). Это
  ОБОСНОВАННОЕ исключение из запрета: явный бросок с понятным сообщением
  вместо тихого NPE.

### 2. `?.` и `?:` — в новом коде не использовать

Заменять на явные `if (x == null) { … } else { … }` / ранние возвраты.
Исключение допускается ТОЛЬКО с inline-обоснованием
`// NULL-ЯВНО: <почему оператор здесь оправдан>`:

```kotlin
val name = user?.first ?: "Гость"
// NULL-ЯВНО: однострочный фолбэк для отображаемого текста,
// null-ветка тривиальна и не несёт логики
```

Критерий исключения: null-ветка тривиальна и не несёт логики (отображаемый
текст, дефолт для UI). Любая ветка с логикой (сеть, кэш, откат состояния) —
только явный `if`.

### 3. Smart-cast свойств классов — захват в локальный `val`
...
if (p == null) {
    ...
    p.x   // smart-cast сработал: p — локальный val
}
```

НИКАКИХ изменений поведения — только форма (см. «Паттерн 1» ниже).

### 4. Легаси мигрирует поэтапно

Существующий код (~6600 вхождений `?.`/`?:` на момент введения политики)
правится поэтапно: Этапы Б/В, по одному модулю за коммит. Новые файлы и
новые правки существующих файлов — сразу по этой политике.
...
Фикс от `029f4e833` сводил `!!` к нулю; с тех пор в легаси-зонах (calls,
api/realtime/auth) остались единичные случаи — они убираются поэтапно
(Этапы Б/В, см. «Явную null-политику» выше). В новом коде и правках — ноль.
...
- [ ] Нет `!!`
- [ ] В новом коде нет `?.`/`?:` без inline-обоснования `// NULL-ЯВНО: …`
      (см. «Явная null-политика #NULL-EXPLICIT» выше)
