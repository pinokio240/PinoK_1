...
class VKApiClient(
     ...
     *
     * Возвращает { base_url, key, ts } для long-poll канала. VK web хранит
     * это в localStorage queue_credential_calls_cache_<uid>_<app_id>.
     * Android-клиент вызывает queue.subscribe напрямую с SAT-токеном.
     *
     * Формат ответа VK:
     ...
     */
    suspend fun queueSubscribe(userId: Long = 0L): QueueCredential? {
        if (isOffline()) return null
        val uid = if (userId > 0L) userId else (exchangeAuthRepository?.userId() ?: 0L)
        val json = call("queue.subscribe", mapOf("queue_ids" to "ACCOUNT_COUNTERS_$uid")) ?: return null
        return try {
            val resp = json.getAsJsonObject("response") ?: return null
            val baseUrl = safeString(resp.get("base_url")) ?: return null
            ...
            val ts = safeLong(first.get("ts"))
            QueueCredential(
                key = key,
                ts = ts,
                url = baseUrl,
                userId = exchangeAuthRepository?.userId() ?: 0L,
            )
        } catch (e: Exception) {
