...
class VKApiClient(
    ...
    override suspend fun queueSubscribe(userId: Long, queueIdSuffix: String?): QueueCredential? {
        ...
        val suffix = queueIdSuffix ?: VKEndpoints.multiaccountQueueId(uid)
        ...
        if (!sat.isNullOrBlank()) {
            try {
                val form = FormBody.Builder()
                    ...
                    .build()
                val req = Request.Builder()
                    .url("${VKEndpoints.QUEUE_SUBSCRIBE_HOST_WEB}/method/queue.subscribe?v=${VKEndpoints.API_VERSION}&client_id=${re.pinok.BuildConfig.VK_WEB_CLIENT_ID}")
                    .post(form)
                    .build()
                httpClient.newCall(req).execute().use { resp ->
