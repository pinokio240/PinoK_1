...
sealed class Screen(val route: String, val title: String, val icon: ImageVector?) {
     ...
     */
    object Call : Screen("call/{peerId}?title={title}&photo={photo}&incoming={incoming}", "Звонок", null) {
        const val ARG_PEER_ID = "peerId"
        const val ARG_TITLE = "title"
        const val ARG_PHOTO = "photo"
        const val ARG_INCOMING = "incoming"
        fun buildRoute(peerId: Long, title: String, photo: String?, incoming: Boolean): String {
            val t = java.net.URLEncoder.encode(title.ifBlank { "Звонок" }, "UTF-8")
            val p = photo?.let { java.net.URLEncoder.encode(it, "UTF-8") } ?: ""
            return "call/$peerId?title=$t&photo=$p&incoming=$incoming"
        }
    }
