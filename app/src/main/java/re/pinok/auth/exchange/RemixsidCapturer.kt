...
object RemixsidCapturer {
    ...
    data class CapturedCookies(
        ...
        // #CALLS-ANTIFRAUD (2026-08-23): антифрод-токены браузерного набора.
        val remixstid: String? = null,      // session anti-fraud token (.vk.ru)
        val remixstlid: String? = null,     // long-lived anti-fraud token (.vk.ru)
        ...
        return withContext(Dispatchers.Main) {
            ...
            if (existing != null) {
                AppLog.i(TAG, "session cookies already in CookieManager " +
                    ...
                    "mvkfp=${if (existing.remixmvkFp != null) "yes" else "no"}, " +
                    "stid=${if (existing.remixstid != null) "yes" else "no"}, " +
                    "stlid=${if (existing.remixstlid != null) "yes" else "no"}) — no WebView needed")
            ...
            try {
                ...
                if (found != null) {
                    AppLog.i(TAG, "capture: SUCCESS — remixsid found (len=${found.remixsid.length}), " +
                        ...
                        "stid=${if (found.remixstid != null) "yes" else "no"}, " +
                        "stlid=${if (found.remixstlid != null) "yes" else "no"}")
                } else {
    ...
    private fun readAllCookiesFromCookieManager(): CapturedCookies? {
        ...
        var remixstid: String? = null
        var remixstlid: String? = null
            ...
            try {
                ...
                for (cookie in cookies) {
                    ...
                    when (name) {
                        ...
                        // #CALLS-ANTIFRAUD (2026-08-23)
                        "remixstid" -> {
                            // сессионный антифрод-токен (<uid>_<token>, .vk.ru)
                            if (remixstid == null && value.length >= 20) {
                                remixstid = value
                            }
                        }
                        "remixstlid" -> {
                            // long-lived антифрод-токен (<uid>_<token>, .vk.ru)
                            if (remixstlid == null && value.length >= 20) {
                                remixstlid = value
                            }
                        }
                    }
     ...
     * все remix-куки + httoken + p со всех VK-доменов, склеенные в "k=v; k=v".
     * Нужен для messages.getCallToken / других методов, требующих авторизации
     * как у веб-приложения (m.vk.ru SPA).
     *
     * @return Cookie-заголовок (может быть пустым если cookies нет).
     */
    fun buildVkCookieHeader(): String {
        return try {
            val cm = CookieManager.getInstance()
            val all = LinkedHashMap<String, String>()
            val urls = AuthDomainsConfig.vkCookieUrls()
            for (url in urls) {
                try {
                    ...
                    for (pair in raw.split(";")) {
                        ...
                        if (parts.size == 2 && parts[0].isNotBlank() && parts[1].isNotBlank()) {
                            // .m.vk.ru / .login.vk.ru дубли — не перезаписываем уже найденные
                            if (!all.containsKey(parts[0])) {
                                all[parts[0]] = parts[1]
                            }
                        }
                    }
